...
async function bootstrap() {
  ...
  // Always allow localhost dev origins so developers can hit the deployed API.
  ...
  const configuredOrigins = frontendUrls.split(',').map(url => url.trim()).filter(Boolean);
  const explicitOrigins = new Set([...configuredOrigins, ...defaultDevOrigins]);

  // Allow Azure Static Web Apps preview deployments
  // and any other origin explicitly listed in FRONTEND_URL.
  const azurePreviewRegex = /^https:\/\/[a-z0-9-]+\.\d+\.azurestaticapps\.net$/i;

  logger.log(`🌐 Allowed CORS origins (explicit): ${JSON.stringify([...explicitOrigins])}`);
  logger.log(`🌐 Also allowing any *.azurestaticapps.net preview matching: ${azurePreviewRegex}`);
  ...
  app.enableCors({
    origin: (origin, callback) => {
      ...
      if (azurePreviewRegex.test(origin)) return callback(null, true);
